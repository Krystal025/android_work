...
/*
    ...
    GET 방식으로 http://아이피주소:9000/boot07/android/tweet 에 요청을 하는 예제
    요청 파라미터는 msg 라는 파라미터 명으로 입력한 문자열이 전송되도록 함
...
public class MainActivity extends AppCompatActivity implements Util.RequestListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sendBtn.setOnClickListener(view->{
            ...
            //요청 파라미터로 전달하기 위해 "msg" 라는 키값으로 Map 에 담음
            Map<String, String> map = new HashMap<>();
            map.put("msg", msg);
            //Util 을 이용해서 http 요청을 한다.
            Util.sendGetRequest(0,
                    "http://192.168.0.17:9000/boot07/android/tweet",
                    map,
                    this);
        });
        // 두번째 버튼에 리스너 등록
        ...
        sendBtn3.setOnClickListener(view->{
            ...
            Util.sendGetRequest(2,
                    "http://192.168.0.17:9000/boot07/android/tweet3",
                    map,
                    this);
        });
        // 글 목록 버튼 리스너 등록
        ...
        listBtn.setOnClickListener(view -> {
            Util.sendGetRequest(3,
                    "http://192.168.0.17:9000/boot07/android/list",
                    null,
                    this);
        });
    }
    ...
    public void onSuccess(int requestId, Map<String, Object> result) {
        if(requestId == 0){
            ...
            //{"isSuccess":true} 형식의 json 문자열은 JSONObject 객체를 이용해서 원하는 데이터를 추출할 수 있음
            try {
                //JSONObject 를 생성해서
                JSONObject obj = new JSONObject(data);
                //"isSuccess" 라는 키값으로 저장된  true 라는 boolean type 데이터 얻어냄
            ...
            String data = (String) result.get("data");
            //data : [] 형식의 json 문자열 -> [] 형식의 json 문자열 : JSONArray 객체를 활용함
            try {
                JSONArray arr = new JSONArray(data);
                for (int i = 0; i < arr.length(); i++) {
                    //i번째 방에 있는 String 타입을 얻어내서
            ...
            //data : [{}, {}, {}, ...] 형식의 json 문자열
            String data = (String) result.get("data");
            //[]형식의 문자열이기 때문에 JSONArray 객체를 생성해야함
            try {
                JSONArray arr = new JSONArray(data);
                for(int i=0; i<arr.length(); i++){
                    //{"num":1, "wirter":"xxx", "title":"xxx", ... }
